...
옵션을 요구합니다. 동일한 `nfsvers=3,hard,nolock` 값을 전용 Secret으로 StorageClass에 연결해
`DeleteVolume`의 controller-side mount에도 적용합니다. Collection 소유 StorageClass의 불변 설정이
바뀌면 소유권을 확인한 뒤 Helm upgrade 전에 해당 StorageClass만 재생성합니다.
